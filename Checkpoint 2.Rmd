...
par(mfrow = c(3,3))
ggplot(attendance.data, aes(x=Attendance/1000)) + geom_histogram(bins=24, color = "black", fill = "skyblue") + labs(title = "Histogram of Attendance")
ggplot(attendance.data, aes(x=Fill.Rate)) + geom_histogram(bins=24, color = "black", fill = "pink") + labs(title = ("Histogram of Fill Rate"))
...
hist(attendance.data$Stadium.Capacity/1000, main = "Histogram of Stadium Capacity")
...
**Since the data on the plot are overlapped beacuase they are cummulated over years, it is good to see the relationships between each year of attendance and other factors.

```{r continuous-over-years}
TemMax_Year <- ggplot(attendance.data, aes(x=TMAX, y=Attendance/1000)) + geom_point()
TemMax_Year + geom_text(label = attendance.data$Year)

TemMin_Year <- ggplot(attendance.data, aes(x=TMIN, y=Attendance/1000)) + geom_point()
TemMin_Year + facet_grid(attendance.data$Year)

Snow_Year <- ggplot(attendance.data, aes(x=SNOW, y=Attendance/1000)) + geom_point()
Snow_Year + facet_grid(attendance.data$Year)

Prcp_Year <- ggplot(attendance.data, aes(x=PRCP, y=Attendance/1000)) + geom_point()
Prcp_Year + facet_grid(attendance.data$Year)

Snwd_Year <- ggplot(attendance.data, aes(x=SNWD, y=Attendance/1000)) + geom_point()
Snwd_Year + facet_grid(attendance.data$Year)
```
