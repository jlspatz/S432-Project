...
```{r np-model-1, cache=TRUE}
library(np)
att_np = npreg(Attendance~Current.Wins+Current.Losses+Stadium.Capacity+
                    factor(ConfCoded)+factor(Tailgating)+factor(RankCoded)+
                    factor(OppRankCoded)+TMIN+TMAX, data=select.set)
```
...
```{r estim-gam}
preds.gam.all = predict(att.gam4, newdata=inference.set, se.fit=TRUE) # no interval here
preds.gam = data.frame(
  obs.value = inference.set$Attendance, 
  fit = preds.gam.all$fit,
  fit.se = preds.gam.all$se.fit)
sigma.gam = sqrt(att.gam4$sig2) # annoyingly in a different place
...
```{r, gam-preds-zoom, fig.width=10, fig.height=4} 
pgam + coord_cartesian(xlim=c(0,3e4), ylim=c(0,7e4))
```

```{r estim-gam-2}
preds.gam.all2 = predict(att.gam7, newdata=inference.set, se.fit=TRUE) # no interval here
preds.gam2 = data.frame(
  obs.value = inference.set$Attendance, 
  fit = preds.gam.all$fit,
  fit.se = preds.gam.all$se.fit)
sigma.gam2 = sqrt(att.gam4$sig2) # annoyingly in a different place
...
```{r, gam-preds-2, fig.width=10, fig.height=4} 
pgam2 <- preds.gam2 %>%
  ggplot(aes(x=obs.value,y=fit,color=captured)) + 
  geom_errorbar(aes(ymin=lwr, ymax=upr), width=.1,color='grey') + 
  geom_point(size=.1) + geom_abline(slope=1, intercept = 0, color='green') + 
  scale_color_manual(values=c('red','blue')) +
  xlab('Actual Attendance') + ylab('Predicted Attendance') +
  ggtitle('Additive model 2')
pgam2
```

```{r, gam-preds-2-zoom, fig.width=10, fig.height=4} 
pgam2 + coord_cartesian(xlim=c(0,3e4), ylim=c(0,7e4))
```
...
```{r, att-prf,fig.height=12}
plot(final.att.mdl, pages = 1, scale=0, shade=TRUE, se=2, bty='n', las=1)
```

```{r, fill-prf,fig.height=12}
plot(final.fill.mdl, pages = 1, scale=0, shade=TRUE, se=2, bty='n', las=1)
```

# IV. Final Discussions and Results

In conclusion, a final model was created that used the available data to predict attendance and stadium fill percentages effectively. This model smoothed the current wins and loses of the home team together, to create a variable that represented the team's overall record at the time of the game. The stadium capacity was smoothed, as well as the maximum and minimum temperatures smoothed together. The smoothing of temperature data together improved the model's accuracy compared to models that smoothed the two separately. The variables of whether the school was a top 25 tailgating location, the ranks of both of the teams playing, and if the home team was in a power conference, were added. Whether the school is a top 25 tailgating location proved to be significant, as well as the capacity of the stadium. 

It was interesting to observe, that in general testing of models, that when whether a game was televised was included as a variable, while not significant, there was a slight positive correlation. There is a common claim that the availability of games being televised affects attendance negatively, as people can watch from their preferred locations of their house or a local sports bar for example. However, this was not seen throughout the modeling processes. It is possible, that the increase of televised games and streaming services might hurt attendance throughout the country, but that would need more reasearch and analysis in the future. 
